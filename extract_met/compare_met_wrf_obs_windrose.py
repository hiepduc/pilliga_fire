...
def plot_taylor_native(station, compare):
    ...
    def stats(obs, mod):
        ...
        return r, std_mod, std_obs

    variables = {
        "T2 (°C)": ("T_OBS", "T_WRF", "red"),
        "Wind Speed (m/s)": ("WS_OBS", "WS_WRF", "blue"),
        "RH (%)": ("RH_OBS", "RH_WRF", "green")
    }

    fig = plt.figure(figsize=(8, 8))
    ax = plt.subplot(111, polar=True)

    # Reference point (OBS std axis)
    r0 = 0
    ax.set_theta_direction(-1)
    ax.set_theta_offset(np.pi / 2)

    # Build Taylor geometry
    theta = np.linspace(0, np.pi/2, 100)
    ax.plot(theta, np.ones_like(theta), 'k--', alpha=0.5)
    ax.set_rmax(2)

    for name, (obs_key, mod_key, color) in variables.items():

        if obs_key not in compare or mod_key not in compare:
            continue

        r, std_mod, std_obs = stats(compare[obs_key], compare[mod_key])

        if np.isnan(r):
            continue

        # Convert correlation to angle
        theta_val = np.arccos(np.clip(r, -1, 1))

        ax.plot(theta_val, std_mod / std_obs,
                marker='o',
                color=color,
                label=name)

    ax.set_title(f"Taylor Diagram - {station}", fontsize=14)
    ax.legend(loc="upper right")

    outfile = f"taylor_{station.replace(' ','_')}.png"
    plt.savefig(outfile, dpi=300, bbox_inches="tight")
    plt.show()
    plt.close()
