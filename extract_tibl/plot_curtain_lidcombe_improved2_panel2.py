...
for t in range(len(times)):
    ...
    grad_s = pd.Series(grad).rolling(
        3,
        center=True,
        min_periods=1
    ).mean()

    idx = np.argmax(grad_s)
...
ax[0].set_title(
    "CL51 Backscatter + BL1/BL2 + WRF PBLH/TIBL"
)
...
ax[2].set_title("Stability dθ/dz")
...
ax[3].set_title("Vertical Velocity")
...
plt.savefig(
    "Lidcombe_WRF_Ceilometer_4Panel_19to21Dec2023.png",
    dpi=300
)
